...
function configuredOrigins(port: number): Set<string> {
  const configured = process.env.MCP_HTTP_ALLOWED_ORIGINS
    ?.split(",")
...
async function createSession(metaToken: string, threadsToken: string | undefined, sessions: Map<string, Session>): Promise<Session> {
  let session: Session;
  ...
  const server = createMetaMcpServer(new MetaApiClient(metaToken, threadsToken));
...
export async function startHttpServer(metaToken: string, threadsToken: string | undefined): Promise<void> {
  const host = process.env.MCP_HTTP_HOST ?? DEFAULT_HOST;
  const port = envNumber("MCP_HTTP_PORT", DEFAULT_PORT);
  const path = normalizePath(process.env.MCP_HTTP_PATH ?? DEFAULT_PATH);
  ...
  if (!isLoopback(host) && !authToken && process.env.MCP_HTTP_ALLOW_UNAUTHENTICATED !== "true") {
    throw new Error(
      "Refusing to expose MCP HTTP without authentication. Set MCP_HTTP_AUTH_TOKEN, " +
      "or explicitly set MCP_HTTP_ALLOW_UNAUTHENTICATED=true."
    );
  }
  ...
  const httpServer = createServer(async (req, res) => {
    ...
    if (req.url === "/healthz" && req.method === "GET") {
      sendJson(res, 200, { status: "ok", transport: "streamable-http", endpoint: path });
      return;
    }
    ...
    if (!authorizationIsValid(req, authToken)) {
    ...
    if (req.method === "POST") {
      try {
        ...
        sendError(res, 400, error instanceof SyntaxError ? "Request body must be valid JSON" : (error as Error).message);
        return;
      }
    }
    ...
    if (!session && req.method === "POST" && isInitializeRequest(body)) {
      try {
        session = await createSession(metaToken, threadsToken, sessions);
      } catch (error) {
        console.error("MCP HTTP server connection error:", error);
        sendError(res, 500, "MCP server failed to initialize");
        return;
      }
    }

    if (!session) {
      sendError(res, sessionId ? 404 : 400, sessionId ? "Session not found" : "Missing MCP session ID");
      return;
    }
  ...
  console.error(`Meta MCP server listening on http://${host}:${port}${path}`);
